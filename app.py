...
if page == "Home":
    ...
    if st.session_state.submitted and search_query:
        with st.empty():
            loading_gif = "https://media.giphy.com/media/3o7btPCcdNniyf0ArS/giphy.gif"  # tiny spinning wrench
            st.markdown(f'<div style="text-align: center;"><img src="{loading_gif}" width="100"></div>', unsafe_allow_html=True)
        ...
        if isinstance(result_data, dict):
            ...
            st.subheader("👍 Did this fix help?")
            col_up, col_down = st.columns(2)

            with col_up:
                if st.button("👍 Yes", key="rate_up"):
                    st.success("Thanks for your feedback!")

            with col_down:
                if st.button("👎 No", key="rate_down"):
                    st.error("Thanks — we’ll improve it!")
    ...
    st.subheader("🛠️ Share Your Fix With Us!")
    with st.form("submit_fix_form", clear_on_submit=True):
        problem = st.text_input("Problem Title")
        cause = st.text_area("Likely Cause")
        fix = st.text_area("Recommended Fix")
        tools = st.text_input("Tools Needed (comma-separated)")
        materials = st.text_input("Materials Needed (comma-separated)")
        submitted = st.form_submit_button("Submit Fix")

        if submitted:
            st.success("✅ Thanks! Your fix idea has been received.")
